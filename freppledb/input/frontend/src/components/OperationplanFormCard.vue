...
<template>
<!--  <div class="card widget mb-3 ng-scope ng-isolate-scope" data-widget="store.operationplan" v-if="widget[0] == 'operationplan'" id="attributes-operationplan" data="store.operationplan" showoperationplan-drv="">-->
  <div class="card">
    <div class="card-header d-flex align-items-center" data-bs-toggle="collapse" data-bs-target="#widget_operationplanpanel" aria-expanded="false" aria-controls="widget_operationplanpanel">
      ...
      <div class="card-body collapse show" id="widget_operationplanpanel" @show="store.operationplan.id || store.operationplan.reference || store.operationplan.operationplan__reference">
        <table style="table-layout:fixed" class="table table-sm table-hover table-borderless" id="opplan-attributes-drvtable">
          <tbody>
            ...
            <tr v-if="store.operationplan.id !== -1&& store.operationplan.type !== 'STCK'">
            ...
            <tr v-if="store.operationplan.item !== null && store.operationplan.id !== -1">
            ...
            <tr v-if="store.operationplan.item__description !== null && store.operationplan.id !== -1">
              <td></td>
              <td>
            ...
            <tr v-if="store.operationplan.supplier__description !== null && store.operationplan.id !== -1">
              <td></td>
            ...
            <tr v-if="store.operationplan.location !== null && store.operationplan.id !== -1">
            ...
            <tr v-if="store.operationplan.type !== 'STCK'">
              <td>
                <b class="text-capitalize" v-if="store.operationplan.type == 'MO' || store.operationplan.id === -1">{{ttt('start date')}}</b>
                ...
                <small v-if="store.operationplan.colmodel?.startdate || store.operationplan.id === -1">({{ ttt(store.operationplan.colmodel?.startdate.type || 'min') }})</small>
              ...
              <td>
                <input v-if='store.operationplan.id === -1&& store.operationplan.start' class="form-control border-0 bg-white" type="datetime-local" v-model="store.operationplan.start" readonly>
                <input v-if='store.operationplan.id !== -1 && !operationplan.hasOwnProperty("store.operationplan__startdate")' class="form-control" type="datetime-local" v-model="store.operationplan.start"
                       @input="setEditValue('setTo', $event.target.value)" :readonly="!editable">
                <input v-if='store.operationplan.id !== -1&& store.operationplan.hasOwnProperty("store.operationplan__startdate")' class="form-control" type="datetime-local" v-model="store.operationplan.operationplan__startdate" @input="setEditValue('setTo', $event.target.value)"" :readonly="!editable">
              </td>
            </tr>
            <tr v-if="store.operationplan.setupend">
              <td><b class="text-capitalize">{{ttt('setup end date')}}</b></td>
              <td>{{(store.operationplan.setupend|| store.operationplan.operationplan__setupend)|formatdatetime}}</td>
            </tr>
            <tr v-if="store.operationplan.type !== 'STCK'">
              <td>
                <b class="text-capitalize" v-if="store.operationplan.type == 'MO' || store.operationplan.id === -1">{{ttt('end date')}}</b>
                ...
                <small v-if="store.operationplan.colmodel?.enddate || store.operationplan.id === -1">({{ ttt(store.operationplan.colmodel?.enddate.type || 'max') }})</small>
              ...
              <td>
                <input v-if='store.operationplan.id === -1 && store.operationplan.end' class="form-control border-0 bg-white" type="datetime-local" v-model="store.operationplan.end" readonly>
                <input v-if='store.operationplan.id !== -1 && !store.operationplan.hasOwnProperty("store.operationplan__enddate")' class="form-control" type="datetime-local" v-model="store.operationplan.end" @input="setEditValue('setTo', $event.target.value)"" :readonly="!editable">
                <input v-if='store.operationplan.id !== -1 && store.operationplan.hasOwnProperty("store.operationplan__enddate")' class="form-control" type="datetime-local" v-model="store.operationplan.operationplan__enddate" @input="setEditValue('setTo', $event.target.value)"" :readonly="!editable">
              </td>
            </tr>
            <tr>
              <td><b class="text-capitalize">{{ttt('quantity')}}</b>&nbsp;
                <small v-if="store.operationplan.id === -1 && store.operationplan.colmodel && !store.operationplan.colmodel['operationplan__quantity']">({{ ttt(store.operationplan.colmodel.quantity.type) }})</small>
                <small v-if="store.operationplan.id === -1 && store.operationplan.colmodel && store.operationplan.colmodel['operationplan__quantity']">({{ ttt(store.operationplan.colmodel.operationplan__quantity.type) }})</small>
              </td>
              <td>
                <span v-if="store.operationplan.id === -1">{{ formatNumber(store.operationplan.operationplan__quantity || store.operationplan.quantity || 0) }}</span>
                <input v-if='store.operationplan.id !== -1 && !store.operationplan.hasOwnProperty("store.operationplan__quantity")' class="form-control" type="number" v-model="store.operationplan.quantity" @input="setEditValue('setTo', $event.target.value)"" :readonly="!editable">
                <input v-if='store.operationplan.id !== -1 && store.operationplan.hasOwnProperty("store.operationplan__quantity")' class="form-control" type="number" v-model="store.operationplan.operationplan__quantity" @input="setEditValue('setTo', $event.target.value)"" :readonly="!editable">
              </td>
            </tr>
            <!--
            <tr v-if="store.operationplan.type === 'MO' || (operationplan.id === -1 && (operationplan.colmodel?.quantity_completed|| store.operationplan.colmodel?.operationplan__quantity_completed))">
              <td><b class="text-capitalize">{{ttt('quantity completed')}}</b>&nbsp;
                <small v-if="store.operationplan.id === -1 && !operationplan.colmodel?.operationplan__quantity_completed">({{ ttt(operationplan.colmodel?.quantity_completed.type) }})</small>
                <small v-if="store.operationplan.id === -1&& store.operationplan.colmodel?.operationplan__quantity_completed">({{ ttt(operationplan.colmodel?.operationplan__quantity_completed.type) }})</small>
              </td>
              <td>
                <span v-if="store.operationplan.id === -1">{{(operationplan.operationplan__quantity_completed|| store.operationplan.quantity_completed || 0)|number}}</span>
                <input v-if='operationplan.id !== -1 && !operationplan.hasOwnProperty("store.operationplan__quantity_completed")' class="form-control" type="number" v-model="store.operationplan.quantity_completed" @input="setEditValue('setTo', $event.target.value)"" :readonly="!editable">
                <input v-if='operationplan.id !== -1&& store.operationplan.hasOwnProperty("store.operationplan__quantity_completed")' class="form-control" type="number" v-model="store.operationplan.operationplan__quantity_completed" @input="setEditValue('setTo', $event.target.value)"" :readonly="!editable">
              </td>
            </tr>
            -->
            <tr v-for="([key, value]) in filteredColmodel" :key="key">
              ...
              <td v-if="['number', 'color', 'currency', 'currencyWithBlanks'].includes(value['formatter'])">{{formatNumber(operationplan[key])}}</td>
              <td v-if="value['formatter'] === 'date'">{{formatdatetime(operationplan[key])}}</td>
            ...
            <tr id="statusrow" v-if="store.operationplan.type !== 'STCK'">
              ...
              <td>
                <div class="btn-group" role="group">
                  <button id="proposedBtn" v-if="(!editable && store.operationplan.status === ttt('proposed')) || editable" type="button" class="btn btn-primary" :class="['text-capitalize', {'active': store.operationplan.status == 'proposed'}]" @click="store.operationplan.status = 'proposed'" :disabled="actions.hasOwnProperty('erp_incr_export') || !editable" data-bs-toggle="tooltip" title="{{'proposed')}}"> <i class="fa fa-unlock"></i></button>
                  <button id="approvedBtn" v-if="(!editable && store.operationplan.status === ttt('approved')) || editable" type="button" class="btn btn-primary" :class="['text-capitalize', {'active': store.operationplan.status == 'approved'}]" @click="store.operationplan.status = 'approved'" :disabled="actions.hasOwnProperty('erp_incr_export') || !editable" data-bs-toggle="tooltip" title="{{'approved')}}"><i class="fa fa-unlock-alt"></i></button>
                  <button id="confirmedBtn" v-if="(!editable && store.operationplan.status === ttt('confirmed')) || editable" type="button" class="btn btn-primary" :class="['text-capitalize', {'active': store.operationplan.status == 'confirmed'}]" @click="store.operationplan.status = 'confirmed'" :disabled="actions.hasOwnProperty('erp_incr_export') || !editable" data-bs-toggle="tooltip" title="{{'confirmed')}}"><i class="fa fa-lock"></i></button>
                  <button id="completedBtn" v-if="(!editable && store.operationplan.status === ttt('completed')) || editable" type="button" class="btn btn-primary" :class="['text-capitalize', {'active': store.operationplan.status == 'completed'}]" @click="store.operationplan.status = 'completed'" :disabled="actions.hasOwnProperty('erp_incr_export') || !editable" data-bs-toggle="tooltip" title="{{'completed')}}"><i class="fa fa-check"></i></button>
                  <button id="closedBtn" v-if="(!editable && store.operationplan.status === ttt('closed')) || editable" type="button" class="btn btn-primary" :class="['text-capitalize', {'active': store.operationplan.status === 'closed'}]" @click="store.operationplan.status = 'closed'" :disabled="actions.hasOwnProperty('erp_incr_export') || !editable" data-bs-toggle="tooltip" title="{{'closed')}}"><i class="fa fa-times"></i></button>
