...
class Command(BaseCommand):
  ...
  statements = [
      ("export_purchaseorder.csv", "COPY (select source, lastmodified, id, status , reference, quantity, to_char(startdate,'YYYY-MM-DD HH24:MI:SS') as startdate, to_char(enddate,'YYYY-MM-DD HH24:MI:SS') as enddate, criticality, owner_id, item_id, location_id, supplier_id, demand_id, name, to_char(due,'YYYY-MM-DD HH24:MI:SS') as due from operationplan where status='proposed' and type='PO') TO STDOUT WITH CSV HEADER"),
      ("export_distributionorder.csv", "COPY (select source, lastmodified, id, status, reference, quantity, to_char(startdate,'YYYY-MM-DD HH24:MI:SS') as startdate, to_char(enddate,'YYYY-MM-DD HH24:MI:SS') as enddate, criticality, plan, destination_id, item_id, origin_id, type, demand_id, name, to_char(due,'YYYY-MM-DD HH24:MI:SS') as due from operationplan where status='proposed' and type='DO') TO STDOUT WITH CSV HEADER"),
      ("export_manufacturingorders.csv", "COPY (select source, lastmodified, id , status ,reference ,quantity , to_char(startdate,'YYYY-MM-DD HH24:MI:SS') as startdate, to_char(enddate,'YYYY-MM-DD HH24:MI:SS') as enddate, criticality, operation_id, owner_id, plan, item_id, type, demand_id, name, to_char(due,'YYYY-MM-DD HH24:MI:SS') as due from operationplan where status='proposed' and type='MO') TO STDOUT WITH CSV HEADER")
      ]
