...
class DataLoadTest(TestCase):
  ...
  def test_forms(self):
    ...
    self.assertEqual(
      PurchaseOrder.objects.all().filter(
        reference="test PO", status="confirmed", quantity=1.0
        ).count(),
      1
      )
    ...
    self.assertEqual(response.status_code, 302)
    self.assertEqual(
      ManufacturingOrder.objects.all().filter(
    ...
    self.assertEqual(response.status_code, 302)
    self.assertEqual(
      DistributionOrder.objects.all().filter(
        reference="test DO", status="confirmed", quantity=1.0, origin=loc1, destination=loc2
        ).count(),
      1
      )
