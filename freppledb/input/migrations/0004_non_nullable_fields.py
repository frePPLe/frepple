...
def CheckReadyForMigration(apps, schema_editor):
    ...
    cnt1 = (
        Buffer.objects.all()
        .using(db)
        .filter(Q(item__isnull=True) | Q(location__isnull=True))
        .count()
    )
    cnt2 = (
        Buffer.objects.values("item", "location")
        .annotate(Count("name"))
...
class Migration(migrations.Migration):
    ...
    dependencies = [("input", "0003_fence_for_po_and_do")]
    ...
    operations = [
        ...
        migrations.AlterField(
            ...
            name="customer",
            field=models.ForeignKey(
                verbose_name="customer", to="input.Customer", on_delete=models.CASCADE
            ),
        ),
        migrations.AlterField(
            model_name="demand",
