  ...
  try {
    ...
    if (loglevel > 0) {
      logger << "Planning demand '" << salesorder
             ...
             << salesorder->getDue();
      if (isGroup) logger << ", " << salesorder->getQuantity();
    ...
    if (isGroup) {
      ...
      for (auto m = salesorder->getMembers(); m != Demand::end(); ++m)
        if (m->getQuantity() > 0 && (m->getStatus() == Demand::STATUS_OPEN ||
    ...
    for (auto l : salesorderlines) {
      ...
      do {
        do {
          ...
          // quantity We don't accept the answer in case: 1) Nothing is planned
