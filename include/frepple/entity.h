/***************************************************************************
  file : $HeadURL: file:///develop/SVNrepository/frepple/trunk/include/frepple/entity.h $
  ...
  (XMLInput& pIn, XMLElement& pElement)
{
  if (pElement.isA(Tags::tag_owner) ||
    ...
    pIn.readto( MetaCategory::ControllerString<T>(T::metadata,pIn.getAttributes()) );
}
...
{
  unsigned short i(0);
  for(T* p = this; p->parent; p = p->parent) ++i;
  return i;
}
