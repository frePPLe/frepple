...
class exporter(object):
  ...
  def export_locations(self):
    ...
    if ids:
      yield '<!-- warehouses -->\n'
      yield '<locations>\n'
      fields = ['name', 'wh_input_stock_loc_id', 'wh_output_stock_loc_id', 'wh_pack_stock_loc_id']
      for i in m.read(ids, fields, self.req.session.context):
  ...
  def export_items(self):
    ...
    if ids:
      ...
      for i in data:
        tmpl = self.product_templates[i['product_tmpl_id'][0]]
        if tmpl['purchase_ok'] and tmpl['supply_method'] == 'buy':  # TODO odoo now uses routes
  ...
  def export_boms(self):
    ...
    m = self.req.session.model('mrp.bom')         # TODO There is now a link between mrp.bom and product.template. 1 template can have multiple boms.
    ids = m.search([('bom_id', '=', False)], context=self.req.session.context)  # TODO odoo splits boms sensibly in 2 tables: mrp.bom and mrp.bom_lines
    fields = [
