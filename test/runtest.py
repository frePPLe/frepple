...
def runTestSuite():
    ...
    if len(tests) == 0:
        ...
        for subdir in subdirs:
            if subdir == '.svn' or os.path.isfile(subdir):
                continue
            tests.append(subdir)
    else:
        # A list of tests has been specified, and we now validate it
        for test in tests:
            if not os.path.isdir(os.path.join(testdir, test)):
                print("Warning: Test directory " + test + " doesn't exist")
                tests.remove(test)
    ...
    for test in tests:
        ...
        tmp = os.path.join(testdir, test, test)

        # Only GCC runs compiled tests
        if len(glob.glob(os.path.join(testdir, test, '*.cpp'))) > 0 and platform != "GCC":
            continue

        # Check the test type
        if os.path.isfile(tmp) or os.path.isfile(tmp + '.exe'):
            # Type 1: (compiled) executable
            AllTests.addTest(FreppleTest(test, 'runExecutable'))
        elif os.path.isfile(os.path.join(testdir, test, 'runtest.py')):
            # Type 2: Python script runtest.py available
            AllTests.addTest(FreppleTest(test, 'runScript'))
        elif os.path.isfile(tmp + '.xml') or os.path.isfile(tmp + '.py'):
            # Type 3: input XML or Python file specified
            AllTests.addTest(FreppleTest(test, 'runXML'))
        else:
            # Undetermined - not a test directory
            print("Warning: Unrecognized test in directory " + test)
