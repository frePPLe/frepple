...
class exporter(object):
    ...
    def load_company(self):
        ...
        for i in m.read(ids, fields, self.req.session.context):
            ...
            self.mfg_location = (
                i["manufacturing warehouse"]
                and i["manufacturing warehouse"][1]
                or self.company
            )
            ...
            for i in data:
                ...
                supply_buy = (
                    len(
                        filter(
                            lambda r: r["name"] == "Buy",
                            map(lambda x: stock_location_routes[x], tmpl["route_ids"]),
                        )
                    )
                    > 0
                )
                if (
                    tmpl["purchase_ok"] and supply_buy
                ):  # tmpl['supply_method'] == 'buy':  # odoo now uses routes
                    for j in self.warehouses:
                        ...
                        yield '<buffer name=%s leadtime="P%sD" xsi:type="buffer_procure"><item name=%s/><location name=%s/></buffer>\n' % (
                            quoteattr(buf),
                            int(tmpl["produce_delay"] + self.po_lead),
                            quoteattr(self.product_product[i["id"]]["name"]),
                            quoteattr(j),
                        )
            yield "</buffers>\n"
...
"""
        ...
        product_ids = product.search(
            [("bom_ids", "!=", False)], context=self.req.session.context
        )
        ...
        for i in m.read(ids, fields, self.req.session.context):
            ...
            if i["product_tmpl_id"][0] in product_bom:
                ...
                if i["id"] in product_bom[i["product_tmpl_id"][0]][0][0]:
                    ...
                    buffer_bom[i["product_tmpl_id"][0]] = [
                        (str(buf_name), product_buf["name"], location)
                    ]
            yield "</buffer>\n"
...
"""
        ...
        ids = m.search(
            [("state", "in", ["draft", "confirmed"])], context=self.req.session.context
        )
        ...
        for i in so_line:
            ...
            if j["picking_ids"]:
                ...
                pick_number = 0
                for p in pick.read(
                    j["picking_ids"], p_fields, self.req.session.context
                ):
                    p_ids = p["move_lines"]
                    product_id = i["product_id"][0]
                    mv_ids = move.search(
                        [("id", "in", p_ids), ("product_id", "=", product_id)],
                        context=self.req.session.context,
                    )
...
"""
        ...
        for i in po_line:
            ...
            location = (
                j["location_id"]
                and self.map_locations.get(j["location_id"][0], None)
                or None
            )
            if location and item and j["state"] == "approved" and not j["shipped"]:
                ...
                if buf not in deliveries and buf not in self.procured_buffers:
...
"""
        ...
        ids = m.search(
            ["|", ("state", "=", "in_production"), ("state", "=", "confirmed")],
            context=self.req.session.context,
        )
        fields = [
            "bom_id",
