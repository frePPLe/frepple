...
'''
        ...
        for i in so_line:
            ...
            if i['state'] == 'draft':
                yield '<demand name=%s quantity="%s" due="%s" priority="%s" minshipment="%s" maxlateness="P0D" status="quote"><item name=%s/><customer name=%s/><location name=%s/></demand>\n' % (
...
#                 else demand status is open
                ...
                for p in pick.read(j['picking_ids'], p_fields, self.req.session.context):
                    ...
                    for mv in move.read(mv_ids, m_fields, self.req.session.context):
                        ...
                        yield '<demand name=%s quantity="%s" due="%s" priority="%s" minshipment="%s" maxlateness="P0D" status="%s"><item name=%s/><customer name=%s/><location name=%s/>\n' % (
